/*
    ...
    from > join(추가 테이블 데이터 가져오기) > where > group by > having > select > order by > limit
*/
...
# >>> null이 '값이 없음'을 나타내기 때문에 그 어떤 값과 비교하거나 연산이 불가
...
# A is not null
# : null이 아닌 경우 true 반환, 인 경우 false 반환
select * from `members`
where
	points is null;
...
select * from `members`
where
	name like 'J%'; # 시작만 J이고 뒤는 0개 이상의 문자를 허용

select * from `members`
where
	name like 'J___'; # 시작이 J이고 뒤는 3개의 임의의 문자를 허용 / Joon
    
select * from `members` # 어떤 문자열 내에서든 un이 포함만 되면 허용
where
	name like '%un%';
